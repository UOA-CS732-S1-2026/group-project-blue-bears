...
const WORDS_PATH = path_1.default.join(__dirname, '../data/commonWords.json');
const WORD_LIST = JSON.parse(fs_1.default.readFileSync(WORDS_PATH, 'utf-8'));
...
const buildRoomPlayerPayload = (player) => ({
    userId: player.userId,
    displayName: player.displayName,
    avatarKind: player.avatarKind,
    ready: player.ready,
});
const normalizePlayerMetadata = (payload) => {
...
const emitRaceResults = async (io, room) => {
    ...
    const [p1, p2] = results;
    if (p1 && p2 &&
        mongoose_1.default.Types.ObjectId.isValid(p1.userId) &&
        mongoose_1.default.Types.ObjectId.isValid(p2.userId)) {
        const winnerId = p1.wpm > p2.wpm ? p1.userId :
...
const registerSocketHandlers = (io) => {
    io.on('connection', (socket) => {
        ...
        socket.on('race_complete', (payload) => {
            const room = rooms.get(payload.roomId);
            if (!room || !room.raceStarted) {
                return;
            }
